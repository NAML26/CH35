...
fetch("https://jsonplaceholder.typicode.com/posts", {
    ...
    method: "POST", 
    //Crear body de nuevo objeto, siguiendo las mismas llaves de la API existente.
    body: JSON.stringify({
        userId: 1986,
...
//Indico que es una funcion asíncrona con la palabra reservada async antes de los parametros. 
const getUser = async () =>{
    //Para que se cumpla la promesa, no se utiliza then, sino que usamos try. Para errores, seguimos utilizando catch
    try {
...
//Convertir el objeto en notacion JSON y almacenarlo en el localStorage
localStorage.setItem("usuario", JSON.stringify(user));

//Traer el objeto desde el localStorage--Vive en la memoria caché
...
//localStorage permanece ahí hasta que se haga limpieza de navegador, sessionStorage solo vive mientras la sesión del navegador está activa. 